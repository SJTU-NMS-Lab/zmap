...
/* modified */
#define ICMP_LATENCY_FIELDSET_FIELDS \
    {.name = "icmp_responder", \
     .type = "string", \
     .desc = "Source IP of ICMP_UNREACH messages"}, \
    {.name = "icmp_type", .type = "int", .desc = "icmp message type"}, \
    ...
    {.name = "icmp_subnet", \
     .type = "string", \
     .desc = "Subnet IP of ICMP_UNREACH messages"}, \
    {.name = "icmp_subnet_len", .type = "int", .desc = "prefix length of subnet"}, \
    {.name = "icmp_next_digit", .type = "int", .desc = "next digit of ip"}, \
    {.name = "icmp_unreach_str", \
     .type = "string", \
     .desc = "for icmp_unreach responses, the string version of icmp_code (e.g. network-unreach)"}

#define ICMP_LATENCY_FIELDSET_LEN 7
...
static inline unsigned short in_checksum(unsigned short *ip_pkt, int len)
{
	...
	sum = (sum >> 16) + (sum & 0xffff);			// what if exceeds 16 bits again?
