		       ...
		       void **arg_ptr)
{
	...
	make_udp_header(udp_header, zconf.target_port, udp_len);						// udp header made in advance

	if (udp_fixed_payload) {
		void *payload = &udp_header[1];
		memcpy(payload, udp_fixed_payload, udp_fixed_payload_len);					// where payload is added in advance
	}
