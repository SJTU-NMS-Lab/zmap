...
void fs_populate_icmp_from_iphdr_latency(struct ip *ip, size_t len, fieldset_t *fs, struct timespec ts)
{
	...
	if (icmp->icmp_type == ICMP_UNREACH) {
		if (ip_inner->ip_p == IPPROTO_UDP) {
			...
			// uint32_t responder = ntohl(ip->ip_src.s_addr);
			uint32_t responder = ntohl(ip_inner->ip_dst.s_addr);
			int prefix_len = 0;
			int next_digit = 2;
			uint32_t subnet = blocklist_lookup_ip_subnet(responder, &prefix_len, &next_digit);
			fs_add_string(fs, "icmp_subnet", make_ip_str(htonl(subnet)), 1);
			fs_add_uint64(fs, "icmp_subnet_len", prefix_len);
			fs_add_uint64(fs, "icmp_next_digit", next_digit);
		...
		fs_add_constchar(fs, "icmp_subnet", "unknown");
		fs_add_uint64(fs, "icmp_subnet_len", 0);
		fs_add_uint64(fs, "icmp_next_digit", 2);
		fs_add_constchar(fs, "icmp_unreach_str", "unknown");
	}
