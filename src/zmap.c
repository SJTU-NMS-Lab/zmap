...
static void *start_send(void *arg)
{
	...
	set_cpu(s->cpu);
	send_run_separate(s->sock, s->shard);						// start sending packets finally
	free(s);
	return NULL;
}
...
static void *start_recv(void *arg)
{
	...
	set_cpu(r->cpu);
	recv_run(&recv_ready_mutex);						// start receiving packets finally
	return NULL;
}
...
static void *start_mon(void *arg)
{
	...
	set_cpu(mon_arg->cpu);
	monitor_run(mon_arg->it, mon_arg->recv_ready_mutex);			// start monitoring finally
	free(mon_arg);
	return NULL;
}
...
{
	if (zconf.iface == NULL) {						// iface = name of network interface 
	...
	iterator_t *it = send_init();					// initialize global sender zsend, not thread-specific
	if (!it) {
	...
	if (!zconf.dryrun) {
		...
		cpu += 1;
		r = pthread_create(&trecv, NULL, start_recv, recv_arg);				// start receiving thread
		if (r != 0) {
			log_fatal("zmap", "unable to create recv thread");
		}
		for (;;) {									// wait until receiving thread is ready
...
#ifdef PFRING											// Pfring: create a worker
	...
	for (uint8_t i = 0; i < zconf.senders; i++) {
		...
		send_arg_t *arg = xmalloc(sizeof(send_arg_t));					// send_arg_t contains cpu, sock, shard
...
{
	struct gengetopt_args_info args;						// store all parsed commandline info
	...
	init_empty_global_configuration(&zconf);					// sending and receiving configuration
...
// PFRING
#ifdef PFRING
#define MAX_CARD_SLOTS 32768												// Pfring: configuring structure pf with zconf
