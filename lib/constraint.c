...
// Return the leaf pertaining to an address, according to the tree
// starting at given root.  (Note: address must be in host byte order.)
static uint32_t _lookup_ip_subnet(node_t *root, uint32_t address, int *prefix_len, int *next_digit)			// modified
{
	assert(root);
	node_t *node = root;
	...
	*prefix_len = (*prefix_len) + 1;
	uint32_t this, retval, previous;
	if (address & mask) {
		node = node->r;
		this = 1<<(32 - *prefix_len);
	} else {
		node = node->l;
		this = 0;
	}
	previous = _lookup_ip_subnet(node, address, prefix_len, next_digit);
	retval = this + previous;
	return retval;
}

// Return leaf pertaining to an address.
// (Note: address must be in host byte order.)
uint32_t constraint_lookup_ip_subnet(constraint_t *con, uint32_t address, int *prefix_len, int *next_digit)		// modified
{
	assert(con);
	assert((*prefix_len) == 0);
	assert((*next_digit) == 2);
	return htonl(_lookup_ip_subnet(con->root, address, prefix_len, next_digit));
}
